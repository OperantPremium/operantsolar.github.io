...
// Change the timeout here to wait longer to respond back to the request.
defaults <- {
    accessControl = true,
    allowUnsecure = false,
    strictRouting = false,
...
app.onTimeout(function(context) {
    context.send(408, { message = "Operant Imp timed out..." });
});/**
 ...
 * Note: Modify the function name to onData when you want to enable AES encryption.
 */
function onDataWithAes(interest, data, face) {
  ...
  if (debugEnable) consoleLog("<DBUG>ContentKey name " + contentKeyName.toUri() + "</DBUG>");
  if (debugEnable) consoleLog("<DBUG>Got data packet with name " + data.getName().toUri() + "</DBUG>");

  // Now fetch the encrypted content key.
  if (debugEnable) consoleLog("<DBUG>Express contentKeyName " + contentKeyName.toUri()+ "</DBUG>");
  face.expressInterest (contentKeyName, function(interest2, data2) {
       onContentKeyData(interest2, data2, data, contentKeyName); }, onTimeout);
}

/**
 * This is called by the library when a Data packet is received for the
 * expressed Interest. Print the Data packet name and content to the console.
 *Note: NO encryption in this call.
 */
function onData(interest, data, face) {
  if (debugEnable) consoleLog("<DBUG>Got data packet with name " + data.getName().toUri() + "</DBUG>");
  local context = app.getContext(contextId);
  context.send (200, data.getContent().toRawStr());
  if (debugEnable) {
    consoleLog("<DBUG>");
 ...
 * Use this to test FED test vectors.
 */
function onDataTestVector(interest, data, face) {
  local encryptedContent = EncryptedContent();
  encryptedContent.wireDecode(data.getContent());
  local contentKeyName = encryptedContent.getKeyLocator().getKeyName();
  ...
  local context = app.getContext(contextId);
  context.send (200, Blob(encryptedContent.getPayload().buf().slice(0, 16)).toHex());
  if (debugEnable) {
    consoleLog("<DBUG>");
    consoleLog(Blob(encryptedContent.getPayload().buf().slice(0, 16)).toHex());
    consoleLog("</DBUG>");
  }
...
function onContentKeyData(interest, data, contentData, contentKeyName) {
  ...
  if (debugEnable) consoleLog("<DBUG>Got content key data packet with name " + data.getName().toUri() + "</DBUG>");
  ...
  // Use Consumer to decrypt contentData and call the given callback.
  ...
  consumer.decryptContent_(contentData, function(content) {
    if (debugEnable) consoleLog("<DBUG>Content: " + content.toHex() + "</DBUG>");
    local context = app.getContext(contextId);
    context.send (200, ImpUtilities.hexToAscii(content.toHex()));
    if (debugEnable) consoleLog("<DBUG>Hex to ASCII " + ImpUtilities.hexToAscii(content.toHex()) + "</DBUG>");
...
function onTimeout(interest) {
  if (debugEnable) consoleLog("<DBUG>Time out for interest " + interest.getName().toUri() + "</DBUG>");
}
...
function processRequest(body) {
  local face = Face (SquirrelObjectTransport(), SquirrelObjectTransportConnectionInfo(device));

  local impapp = impApplication();
  local name = impapp.getNameforRequest(body);
  local interest = Interest(name);
  interest.setInterestLifetimeMilliseconds(impapp.getInterestLifetime());
  // NOTE: Change the onData function name appropriately to either onDataWithAes or onDataTestVector to 
  // modify what we are testing.
  face.expressInterest
    (interest, function(interest, data) { onData(interest, data, face); }, onTimeout);
}

app.post("/", function (context) {
    if (debugEnable) consoleLog("<DBUG>impconfigid: " + imp.configparams.deviceid + "</DBUG>");
    contextId = context.id;

    if (debugEnable) {
        consoleLog ("<DBUG>Interest =" +
          " usng:" + http.jsondecode(context.req.rawbody).usng + 
          " deviceIdHash: " + http.jsondecode(context.req.rawbody).deviceIdHash +
          " rw:" + http.jsondecode(context.req.rawbody).rw +
          " category:" + http.jsondecode(context.req.rawbody).category +
          " task:" + http.jsondecode(context.req.rawbody).task +
          " parameters:" + http.jsondecode(context.req.rawbody).parameters +
          "</DBUG>");
    }

    processRequest(http.jsondecode(context.req.rawbody));
})
